...
DPad.prototype.placeRelative = function(dx, dy) {
  ...
  style.left = (me.distance.x/me.scale + dx) + "px";
  style.right = (me.distance.x/me.scale - dx) + "px";
  style.top = (me.distance.y/me.scale + dy) + "px";
  style.bottom = (me.distance.y/me.scale - dy) + "px";
};
...
var dpad1 = new DPad("dpad-1", {
    ...
    "directionchange": function(key, pressed) {
      if (!arrayContains(genericCtrl.enabledButtons, key)) return;
      if (pressed) {
        genericCtrl.signalFinalPress(key);
      } else {
        genericCtrl.signalFinalRelease(key);
      }
    },
    "touchstart": function() {
      //
...
genericCtrl.setControllerLayoutExtended = function (keymap) {
  var disable = true;
  var possibleKeys = [DPad.UP, DPad.DOWN, DPad.LEFT, DPad.RIGHT];
  for (i = 0; i < possibleKeys.length; i++) {
    if (arrayContains(genericCtrl.enabledButtons, possibleKeys[i])) {
      disable = false;
      break;
    }
  }
  var dpad_input = document.getElementById("dpad-input");
  if (disable) {
    dpad_input.classList.add("disabled");
  }
  else {
    dpad_input.classList.remove("disabled");
  }
}